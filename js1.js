document.addEventListener('DOMContentLoaded', () => {
  ...
  let paginaAtual = 1;
  const limite = 4; // Limite de séries por página
  let totalPaginas = 0; // Inicialmente 0 para não mostrar paginação
          ...
          fetch(`http://localhost:3000/series?titulo=${encodeURIComponent(titulo)}&pagina=${paginaAtual}&limite=${limite}`)
              ...
              .then(data => {
                  ...
                  if (data && Array.isArray(data.data)) {
                      ...
                      paginationControls.style.display = 'none'; // Esconde a paginação se não houver resultados
                  }
              })
  ...
  function exibirSeries(series) {
      ...
      // Garantir que estamos exibindo apenas o número correto de séries (usando o limite)
      const seriesExibidas = series.slice(0, limite); // Limita o array ao número de itens desejado

      seriesExibidas.forEach(serie => { // Itera apenas sobre as séries exibidas
  ...
  function atualizarPaginacao() {
      ...
      // Mostrar ou esconder a paginação com base no total de páginas
